# from https://github.com/roman-vygon/triplet_loss_kws
...
class online_triplet_loss(nn.Module):
    """
    Online Triplet loss from: https://github.com/roman-vygon/triplet_loss_kws
    ...
    def compute(self, zq, n_sample, n_class):
        # compute targets
        target_inds = torch.arange(0, n_class).view(n_class, 1, 1).expand(n_class, n_sample, 1).long()
        target_inds = Variable(target_inds, requires_grad=False)
        if zq.is_cuda:
            target_inds = target_inds.cuda()
        target_flat = target_inds.view(n_class * n_sample, *target_inds.size()[2:]).squeeze()
        
        triplets = self.triplet_selector.get_triplets(zq, target_flat)

        if zq.is_cuda:
            triplets = triplets.cuda()
            ap_distances = (zq[triplets[:, 0]] - zq[triplets[:, 1]]).pow(2).sum(1) 
            an_distances = (zq[triplets[:, 0]] - zq[triplets[:, 2]]).pow(2).sum(1)
            #print(ap_distances.mean(), an_distances.mean())
            loss_val = F.relu(ap_distances - an_distances + self.margin).mean()

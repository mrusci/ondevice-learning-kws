...
class TCResNet(nn.Module):
    def __init__(self, in_channels, in_height, in_width, n_blocks, n_channels, 
                 conv_kernel, res_kernel, dilation, stride=2, lastnorm=False):
    ...
    def build_resnet(self,in_height, in_width, n_blocks, n_channels, dilation, res_kernel, stride):
        ...
        for i in range(n_blocks):
            ...
            if self.lastnorm and i == n_blocks-1:
                res_blocks.append(('Res_{}'.format(i+1),
                                    ResidualBlockNorm(input_channels, in_height, in_width, 
...
def TCResNet8_NORM(in_c, in_h, in_w, width_multiplier=1.0):
    n_blocks = 3
    n_channels = [16, 24, 32, 48]
    conv_kernel = (3,1)
    res_kernel = (9,1)
    ...
    return TCResNet(in_w, in_h, in_c, n_blocks, n_channels, conv_kernel, res_kernel, dilation, lastnorm=True)
